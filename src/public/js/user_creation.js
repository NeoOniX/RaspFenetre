// Show & Hide password :
const showPasswordBtn = document.querySelector('form#user_creation #show_password');
...
// Change Icon preview :
const logoInputFile = document.querySelector('form#user_creation .icon input[name="icon"]');
const logoImgPreview = document.querySelector('form#user_creation .icon .logo #logo');

logoInputFile.addEventListener('change', (e) => {
    const [file] = logoInputFile.files;
    if (file) {
        logoImgPreview.src = URL.createObjectURL(file);
    }
});
